...
def clipping_value_by_percentage(testing_model, clipping_percentage, activation_index, inputs, positive_values_only=False):
    ...
    layer_outputs = testing_model.get_activation_layer_outputs(
        inputs, activation_index, activation=True)

    if positive_values_only:
        clipping_value = np.percentile(
            layer_outputs[layer_outputs > 0], 100 - clipping_percentage)
    else:
        clipping_value = np.percentile(
            layer_outputs, 100 - clipping_percentage)
...
def clipping_list_by_percentage(testing_model, clipping_percentage, inputs):
    ...
    for i in range(model_copy.activation_nb):
        clipping_value = clipping_value_by_percentage(
            model_copy, clipping_percentage, i, inputs)
        clipping_list.append(clipping_value)
        new_c_relu = model_copy.c_relus_list[i].change_parameter(
...
def compute_range(testing_model, parameter_name, allowed_accuracy_variation, allowed_loss_variation, significant_number_max, significant_number_min, dataset, inputs=None):
    ...
    if not parameter_name in ['beta', 'gamma', 'clipping_percentage']:
        raise AssertionError(
            "Parameter must be 'beta', 'gamma' or 'clipping_percentage'")
    ...
    if (parameter_name == 'clipping_percentage'):
        ...
        while accuracy > main_accuracy - allowed_accuracy_variation and loss < main_loss + allowed_loss_variation:
            ...
            print("Model testing with {} = {}".format(
                parameter_name, parameter))
            _, c_relus_list = clipping_list_by_percentage(
                model_copy, parameter, inputs)
            for i in range(model_copy.activation_nb):
                model_copy.change_c_relu(c_relus_list[i], i)

            loss, accuracy = model_copy.evaluate(dataset)
            parameter = np.round(
                parameter + pow(10, -significant_number_max), significant_number_max)
    else:
        while accuracy > main_accuracy - allowed_accuracy_variation and loss < main_loss + allowed_loss_variation:
            parameter_max = parameter
            print("Model testing with {} = {}".format(
        ...
        while accuracy > main_accuracy - allowed_accuracy_variation and loss < main_loss + allowed_loss_variation:
            ...
            loss, accuracy = model_copy.evaluate(dataset)
            parameter = np.round(
                parameter - pow(10, -significant_number_min), significant_number_min)
    del model_copy
...
def plot_varying_parameter_accuracy(testing_model, parameter, parameter_min, parameter_max, parameter_step, dataset, plot_loss=False):
    ...
    if not parameter in ['beta', 'gamma', 'clipping']:
        ...
        if plot_loss:
            ...
            plt.title(
                'Loss of model as a function of {} - Model : {}'.format(parameter, testing_model.name))
        ...
        plt.title(
            'Accuracy of model as a function of {} - Model : {}'.format(parameter, testing_model.name))
        plt.xlabel(parameter)
        plt.ylabel('accuracy')
        plt.ylim(0, 1)
        plt.legend()
...
def plot_varying_parameter_hist(testing_model, parameter, parameter_min, parameter_max, parameter_step, inputs, activation_index=0, no_0=False):
    ...
    if not parameter in ['beta', 'gamma', 'clipping']:
        ...
        for x in X:
            ...
            outputs_before_activation = model_copy.get_activation_layer_outputs(
                inputs, activation_index, activation=False)
            outputs_after_activation = model_copy.get_activation_layer_outputs(
                inputs, activation_index, activation=True)
            ...
            plt.title("Model distribution of outputs at layer {} with {} = {} - Model : {}".format(
                activation_index, parameter, x, testing_model.name))
...
def plot_varying_clipping_percentage_accuracy(testing_model, clipping_percentage_min, clipping_percentage_max, clipping_percentage_step, dataset, inputs, plot_loss=False):
    if (clipping_percentage_min < 0 or clipping_percentage_max < 0 or clipping_percentage_min > 100 or clipping_percentage_max > 100):
        ...
        X_clipping_percentage = np.arange(
            clipping_percentage_min, clipping_percentage_max, clipping_percentage_step)
        Y_clipping_percentage = []
        Y_clipping_percentage_loss = []

        for x_clipping_percentage in X_clipping_percentage:
            _, c_relus_list = clipping_list_by_percentage(
                testing_model, x_clipping_percentage, inputs)
            for i in range(model_copy.activation_nb):
                model_copy.change_c_relu(c_relus_list[i], i)
            loss, accuracy = model_copy.evaluate(dataset)
        ...
        if plot_loss:
            plt.subplot(2, 1, 2)
            plt.plot(X_clipping_percentage, Y_clipping_percentage_loss,
                     "o", label="Variation of clipping percentage")
            plt.title(
                'Loss of model as a function of clipping percentage - Model : {}'.format(testing_model.name))
        ...
        plt.title(
            "Accuracy of model as a function of clipping percentage - Model : {}".format(testing_model.name))
        plt.xlabel("clipping percentage")
        plt.ylabel("accuracy")
        plt.ylim(0, 1)
        del model_copy
...
def plot_heatmap_varying_parameters_accuracy(testing_model, parameter_list_x, parameter_list_y, dataset, inputs):
    ...
    if not all(parameter in ['beta', 'gamma', 'clipping', 'clipping_percentage'] for parameter in parameters) or parameters[0] == parameters[1] or all(parameter in ['clipping', 'clipping_percentage'] for parameter in parameters):
        raise AssertionError(
            "Invalid parameters, parameters must be different from each other and be 'beta', 'gamma', 'clipping' or 'clipping_percentage'.")
    else:
        _, parameter_x_min, parameter_x_max, parameter_x_step = parameters_matrix[0]
        X_parameter = np.arange(
            parameter_x_min, parameter_x_max, parameter_x_step)

        _, parameter_y_min, parameter_y_max, parameter_y_step = parameters_matrix[1]
        Y_parameter = np.arange(
            parameter_y_min, parameter_y_max, parameter_y_step)
        ...
        for x in X_parameter:
            ...
            for y in Y_parameter:
                print("Model testing with {} = {} and {} = {}".format(
                ...
                if 'clipping_percentage' in parameters:
                    ...
                    if index-1 == 0:
                        new_c_relu = c_relu.change_parameter(
                            parameters[index-1], x)
                        model_copy.change_all_c_relus(new_c_relu)
                        clipping_list, _ = clipping_list_by_percentage(
                            model_copy, y, inputs)
                    else:
                        new_c_relu = c_relu.change_parameter(
                            parameters[index-1], y)
                        model_copy.change_all_c_relus(new_c_relu)
                        clipping_list, _ = clipping_list_by_percentage(
                            model_copy, x, inputs)

                    for i in range(model_copy.activation_nb):
                        new_c_relu = new_c_relu.change_parameter(
                            'clipping', clipping_list[i])
                        model_copy.change_c_relu(new_c_relu, i)
        ...
        p = sns.heatmap(Z, annot=True, annot_kws={"size": 6}, xticklabels=np.round(
            Y_parameter, 2), yticklabels=np.round(X_parameter, 2))
        p.set_xlabel(parameters[1])
        p.set_ylabel(parameters[0])
        plt.title("Accuracy of model as a function of {} and {}".format(
...
def plot_varying_model_HW_dist(testing_model_list, inputs, activation_index=0, diff=False):
    ...
    for testing_model in testing_model_list:
        ...
        outputs_after_activation = testing_model.get_activation_layer_outputs(
            inputs, activation_index, activation=True)
    ...
    hist, xedges, yedges = np.histogram2d(x, y, bins=[32, len(
        testing_model_list)], range=[[0, 32], [0, len(testing_model_list)]])
    ...
    if (diff):
        ...
        ref_outputs_after_activation = testing_model.get_activation_layer_outputs(
            inputs, activation_index, activation=True)
        ...
        plt.title("HW distribution diff of different model with Custom ReLU - layer {} - Model : {}".format(
            activation_index, testing_model_list[0].name))
...
def plot_varying_parameter_HW_dist(testing_model, parameter, parameter_min, parameter_max, parameter_step, inputs,
    ...
    PARAM = np.arange(parameter_min, parameter_max +
    ...
    if not parameter in ['beta', 'gamma', 'clipping']:
        ...
        if no_0:
            hist, xedges, yedges = np.histogram2d(x, y, bins=[31, len(PARAM)], range=[
                                                  [1, 32], [parameter_min, parameter_max+parameter_step]])
        else:
            hist, xedges, yedges = np.histogram2d(x, y, bins=[32, len(PARAM)], range=[
                                                  [0, 32], [parameter_min, parameter_max+parameter_step]])
        ...
        if (diff):
            ...
            ref_outputs_after_activation = testing_model.get_activation_layer_outputs(
                inputs, activation_index, activation=True)
            ref_HW_layer_output = to_HW_layer_output(
                ref_outputs_after_activation)
            nb_bit = np.array([x for x in range(32)])
            counts, bins = np.histogram(ref_HW_layer_output, bins=nb_bit)
            for i in range(len(counts)):
                hist_diff[i] = abs(hist_diff[i]-counts[i])
            dz_diff = hist_diff.ravel()
            ax2.bar3d(xpos, ypos, zpos, dx, dy, dz_diff, zsort='average')
            plt.title("HW distribution diff with {} variation for Model : {}".format(
                parameter, testing_model.name))
            plt.xlabel("hamming weight")
        ...
        plt.title("HW distribution with {} variation for Model : {}".format(
            parameter, testing_model.name))
        plt.xlabel("hamming weight")
        plt.ylabel("{}".format(parameter))
        del model_copy
...
def plot_varying_parameter_HW_diff(testing_model, parameter, parameter_min, parameter_max, parameter_step, inputs,
    ...
    if not parameter in ['beta', 'gamma', 'clipping']:
        ...
        plt.title("HW diff with {} variation for Model : {} at layer {}".format(
            parameter, testing_model.name, activation_index))
        plt.xlabel("hamming weight")
        plt.ylabel("{}".format(parameter))
        del model_copy
        return fig
...
def update_model_c_relus_with_ranges(testing_model, beta_range, gamma_range, clipping_percentage_range, inputs):
    ...
    clipping_percentage_mu, clipping_percentage_sigma = compute_parameter_mu_sigma(
        clipping_percentage_range)
    random_percentage = give_param(
        clipping_percentage_mu, clipping_percentage_sigma, clipping_percentage_range)

    c_relu = CustomReLU(random_beta, random_gamma, np.inf)
    testing_model.change_all_c_relus(c_relu)
    clipping_list, _ = clipping_list_by_percentage(
        testing_model, random_percentage, inputs)
...
def test_model_c_relus_with_ranges(testing_model, beta_range, gamma_range, clipping_percentage_range, dataset, inputs=None):
    ...
    for i in range(10):
        beta, gamma, clipping_percentage = update_model_c_relus_with_ranges(
            model_copy, beta_range, gamma_range, clipping_percentage_range, inputs)
    ...
    plt.title(
        "Model accuracy with random parameters in ranges - Model : {}".format(model_copy.name))
    plt.xlabel("model iteration")
    plt.ylabel("accuracy")
    plt.hlines(ref_accuracy, xmin=0, xmax=10,
               linestyles='dashed', label="Reference accuracy")
    plt.legend()
...
def update_model_c_relus_changing_every_inference(testing_model, beta_range, gamma_range, clipping_percentage_range, inputs, outputs, dataset,
    ...
    plt.xlabel("model iteration")
    plt.ylabel("accuracy")
    plt.hlines(ref_accuracy, xmin=0, xmax=nb_test,
               linestyles='dashed', label="Reference accuracy")
    plt.legend()
    ...
    if not parameter in ['beta', 'gamma', 'clipping']:
        ...
        for sigma in sigma_tab:
            ...
            if parameter == 'beta':
                ...
                for x in X:
                    print("Testing model accuracy with sigma = {} and {} = {}".format(
                        sigma, parameter, x))
                    model_copy.change_all_c_relus(CustomReLU(x, 0, np.inf))
                    _, accuracy = model_copy.evaluate(dataset)
        ...
        plt.title('Accuracy of model as a function of sigma of {} - Model : {}'.format(
            parameter, testing_model.name))
        plt.xlabel('sigma')
        plt.ylabel('accuracy')
        plt.ylim(0, 1)
...
def plot_varying_sigma_clipping_percentage_accuracy(testing_model, sigma_min, sigma_max, sigma_step, mu, dataset):
    ...
    for sigma in sigma_tab:
        ...
        for x_clipping_percentage in X_clipping_percentage:
            clipping_list, c_relus_list = clipping_list_by_percentage(
                testing_model, x_clipping_percentage)
            print(clipping_list)
            for i in range(model_copy.activation_nb):
                model_copy.change_c_relu(c_relus_list[i], i)
            _, accuracy = model_copy.evaluate(dataset)
