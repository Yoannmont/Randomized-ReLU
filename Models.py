...
def plot_hist_diff_layer(x, model_1, model_2, activation_index, no_0=False, activation=False):
    model_1_outputs = model_1.get_activation_layer_outputs(x, activation_index, activation)
    model_2_outputs = model_2.get_activation_layer_outputs(x, activation_index, activation)
    ...
    plt.hist(bins[:-1],bins , weights=counts, label="Difference of outputs distribution")
    plt.title("Difference between model {} and model {} at layer {}".format(model_1.name, model_2.name, activation_index))
...
def to_HW_layer_output(layer_outputs):
    ...
    HW_layer_output = np.squeeze(HW_IEEE754(int32_layer_output).astype(np.uint8)) 
...
class Testing_Model(Model):
    def __init__(self, config, training_model=Training_Model(path=''), c_relus_list=[CustomReLU(1,0,np.inf)]):
        ...
        self.c_relus_matrix = [c_relu.get_parameters() for c_relu in self.c_relus_list]
    ...
    def call(self, x):
        ...
        for i in range (len(self.layers_list)):
            ...
            if isinstance(self.layers_list[i],ReLU):
                x = tf.keras.activations.relu(self.c_relus_matrix[activation_index][0]*(x - self.c_relus_matrix[activation_index][1]), max_value = self.c_relus_matrix[activation_index][2])
                activation_index += 1
            else :
                x = self.layers_list[i](x)
        return Softmax()(x)
    ...
    def clone(self):
        ...
        model_copy = Testing_Model(config = self.config, training_model=self.training_model, c_relus_list=self.c_relus_list)
    ...
    def plot_hist_distrib_layer(self, x, activation_index, no_0=False):
        ...
        outputs_before_activation = self.get_activation_layer_outputs(x, activation_index, activation=False)
        outputs_after_activation = self.get_activation_layer_outputs(x, activation_index, activation=True)
        ...
        plt.title("Model distribution of outputs at layer {} - Model : {}".format(activation_index, self.name))
        plt.xlabel("Outputs values")
        plt.ylabel("# Elements")
        plt.grid(True)
        plt.legend(loc="upper right")
    ...
    def plot_HW_distrib_hist_layer(self, x, activation_index, no_0=False, activation=True):
        layer_outputs = self.get_activation_layer_outputs(x, activation_index, activation)
        ...
        plt.title("Model distribution of hamming weights outputs at layer - Model : {}".format(activation_index, self.name))
